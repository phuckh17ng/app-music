...
import styles from "./MusicPlayerScreen.module.scss";
...
const MusicPlayerScreen = () => {
  ...
  const scrollMusic = () => {
    ...
    if (musicPlayerContainer) {
      if (scrollY > scrollStop) {
        containerDeg = containerDeg + 2.5;
      }
      if (scrollY < scrollStop) {
        containerDeg = containerDeg - 3.5;
      }
    }
    musicPlayerContainer.style.transform = `rotate(${containerDeg}deg)`;
    scrollStop = scrollY;
  };
  ...
  const changeColor = () => {
    ...
    console.log(colorChange?.getBoundingClientRect());
    console.log(height);
